...
```{r top-table, echo = F, message=F, warning=FALSE}
library(tidyverse)
library(fs)
library(glue)
library(kableExtra)
...
rmds <- dir_ls(here::here('slides/lectures'), regex = '\\d{2}-.*.Rmd') %>% 
  path_file()
rmds <- paste0('https://github.com/araastat/BIOF439/blob/master/slides/lectures/',rmds)
htmls <- dir_ls(here::here('docs/slides/lectures'),regex = '\\d{2}-.*.html') %>% 
  path_rel(start = here::here('docs/slides'))
pdfs <- dir_ls(here::here('docs/slides/lectures'), regex = '\\d{2}-.*.pdf') %>% 
  path_rel(start = here::here('docs/slides'))
...
blah <- blah %>% tidyr::gather(variable, value, -base) %>% 
  ...
  mutate(Title = map_chr(path(here::here('slides/lectures'), path_file(str_match(Rmd, '\\((.+)\\)')[,2])),parse_title)) %>% 
