...
.pull-left[
```{r maps-2, echo=FALSE, out.height="70%", out.width="70%",fig.pos='h'}
...
.pull-left[
```{r maps-3, echo=FALSE, fig.caption='Smoking rates in USA in 2012 (NY Times, March 24, 2014)'}
knitr::include_graphics('img/Smoking-rates.png')
```
.small[Smoking rates in USA in 2012 (NY Times, March 24, 2014)]
]
.pull-right[
```{r maps-4, echo=FALSE, out.height="120%", out.width="120%"}
...
```{r maps-6, out.width='60%', out.height='60%'}
ggplot(data = spData::world) + 
  geom_sf(aes(fill = lifeExp)) + # a special geometry for plotting maps
...
.left-column70[
```{r maps-9, out.width='100%', out.height='100%', eval=TRUE}
tmap_mode('view')  #<<
mymap <- tm_shape(spData::world) + tm_polygons(col = 'pop', style='jenks',
                                      palette='Reds', title='Population',
                                      popup.vars = c('pop'), id='name_long')
```
```{r maps9-out, echo=F, eval=T}
tmap_save(mymap, 'map.html')
...
```{r maps-10, eval=TRUE, cache=FALSE}
library(leaflet)
...
.pull-left[
```{r maps-11, echo=TRUE, eval=FALSE}
load(file.path(here::here('slides','lectures','data', 'exdata.rda')))
gpx <- sf::st_as_sf(gpx, 
...
.pull-left[
```{r maps-13, echo=TRUE, eval=FALSE}
load(file.path(here::here('slides','lectures','data', 'exdata.rda')))
gpx <- st_as_sf(gpx, 
                coords=c("Longitude", "Latitude"), 
                crs = 4267) # Need to make sf object
...
.pull-right[
```{r maps-16, echo=FALSE, ref.label='qm1'}
...
.pull-right[
```{r maps-18, echo=FALSE, out.height="100%", out.width="100%"}
