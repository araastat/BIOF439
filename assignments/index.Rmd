...
templates <- dir_ls('HW', glob = '*template.rmd')
hws <- dir_ls('HW', glob = 'HW*.html')
hws <- setdiff(setdiff(hws, solutions), templates)
...
# out_data <- tibble(n = numbers, hws = sort(hws), solutions = solutions[ind])

# out <- tibble('Homework' = glue_data(out_data,'[Homework {n}]({hws})'),
#               # 'R Markdown Template' = glue_data(out_data,
