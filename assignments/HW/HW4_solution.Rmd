...
plt1 <- ggplot(riskfactors) + geom_histogram(aes(x=age))
plt2 <- ggplot(riskfactors) + geom_bar(aes(x = sex))
plt3 <- ggplot(riskfactors) + geom_boxplot(aes(x = education, y = bmi))
...
```{r, message=FALSE}
library(coronavirus)
countries_10 <- coronavirus %>% 
  filter(type=='death') %>% 
  filter(date=='2020-07-06') %>% 
  group_by(country) %>% 
...
library(coronavirus)
coronavirus %>% filter(date=='2020-07-06', type=='death') %>% group_by(country) %>% summarise(cases = sum(cases, na.rm=T)) %>% ungroup() %>% slice_max(cases, n=10) %>% pull(country)
```
