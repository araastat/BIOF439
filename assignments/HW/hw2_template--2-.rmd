...
*Answer: If you just use "import("data/gapminder.csv")", RStudio will output the data in the console but will not assign it to any variable in the workspace*
...
ggplot(data = gapm, 
  ...
  ggtitle("Global Population Changes (1957-2007)")
```
...
```{r HW2-10}
ggplot(gapm, aes(x = gdpPercap, y = lifeExp, size = sqrt(pop), color = continent)) + 
  geom_point() +
  scale_x_log10() +
  facet_wrap(~year)
```
...
```{r HW2-11}
ggplot(gapm, aes(gdpPercap, lifeExp, size = pop, colour = continent)) +
    geom_point(alpha = 0.5, show.legend = TRUE) +
    scale_size(range = c(2, 12)) +
    scale_x_log10() +
    labs(title = 'Year: {frame_time}', x = 'Per capita GDP', y = 'Life expectancy') +
